...
export default function ValuePickerDesktop({ values, className="" }) {
    ...
    return (
        <div id="difficultyControlsCtn" className={` ${className}`}>
            ...
            {values.map(valueI =>
                <ValuePickerValueDesktop key={valueI.value} value={valueI} checked={selectedValue === valueI} clickHandler={() => setSelectedValue(valueI)} />
            )}
        </div>
    );
}

function ValuePickerValueDesktop({ value, checked, clickHandler }) {
    return (
